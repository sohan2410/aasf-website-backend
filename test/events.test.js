...
describe('Event Actions', function() {
  ...
  it('should get events', async () => {
    ...
    expect(response.body.events[0].numberOfDays).to.be.a('number');
  ...
  it('should allow to add winners', async () => {
    ...
    let response = await request(Server)
      ...
      .set('Authorization', `bearer ${token}`)
      .send({
        winners: [['2020BCS-044'], ['2020BCS-031', '2020BCS-050']],
        eventId: event._id,
      });
    ...
    response = await request(Server)
      ...
      .set('Authorization', `bearer ${token}`)
      .send({
        winners: [['2020BCS-044'], ['2020BCS-031', '2020BCS-055']],
        eventId: '615f758ed92d2b5ee1975eb8',
      }); //using invalid roll number and event id
